...
def load_tkhtml(main, location=None):
    global _tkhtml_loaded
    if not _tkhtml_loaded:
        if location:
            main.tk.eval('global auto_path; lappend auto_path {%s}' % location)
        main.tk.eval('package require Tkhtml')
        _tkhtml_loaded = True        
...
class TkinterHtml(tk.Widget):
    def __init__(self, main, cfg={}, **kw):
        #print(get_tkhtml_folder())
        load_tkhtml(main, get_tkhtml_folder())
        tk.Widget.__init__(self, main, 'html', cfg, kw)
...
class HtmlFrame(ttk.Frame):
    def __init__(self, main, fontscale=0.8, vertical_scrollbar=True,
